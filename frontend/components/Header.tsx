...
const StatItem = ({ label, value, diff, isCompact }: { label: string, value: string | number, diff?: string | number, isCompact?: boolean }) => {
  ...
  return (
    ...
    <div className={`flex flex-col items-center flex-1 min-w-0 transition-all duration-300 ${isCompact ? 'px-0.5' : 'px-1 lg:px-2'}`}>
      <span className={`text-fgSubtle uppercase tracking-wider font-bold mb-0.5 whitespace-nowrap transition-all duration-300 ${isCompact ? 'text-[8.5px]' : 'text-[9.5px] lg:text-[9px]'}`}>{label}</span>
      <span className={`font-bold text-white leading-none mb-0.5 transition-all duration-300 ${isCompact ? 'text-[15px]' : 'text-[16px] lg:text-[18px]'}`}>{typeof value === 'number' ? value.toFixed(1) : value}</span>
