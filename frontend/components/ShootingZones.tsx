...
export const ShootingZones: React.FC<ShootingZonesProps> = ({ player }) => {

  const zones = useMemo(() => {
    // Fallback / Mock Data if player data missing
    // In a real scenario, we'd parse this from player.stats or a separate endpoint
    return {
      leftCorner: { pct: '2%', val: '19' },
      rightCorner: { pct: '2%', val: '14' },
      rim: { pct: '49%', val: '25' },
      paint: { pct: '15%', val: '2' },
      midRange: { pct: '11%', val: '26' },
      topKey: { pct: '21%', val: '10' }
    };
  }, [player]);
  ...
  return (
    <div className="bg-card rounded-lg p-5 w-full">
      ...
      <div className="relative w-full aspect-[1.3] max-w-[320px] mx-auto mt-6">
        ...
        {/* Left Corner */}
        <ZoneLabel top="16%" left="8%" pct={zones.leftCorner.pct} val={zones.leftCorner.val} />
        {/* Right Corner */}
        <ZoneLabel top="16%" left="92%" pct={zones.rightCorner.pct} val={zones.rightCorner.val} />

        {/* Paint / Rim */}
        <ZoneLabel top="16%" left="50%" pct={zones.rim.pct} val={zones.rim.val} />

        {/* High Post / FT Circle */}
        <ZoneLabel top="45%" left="50%" pct={zones.paint.pct} val={zones.paint.val} />

        {/* Mid Range Center */}
        <ZoneLabel top="68%" left="50%" pct={zones.midRange.pct} val={zones.midRange.val} />

        {/* Top of Key 3PT */}
        <ZoneLabel top="88%" left="50%" pct={zones.topKey.pct} val={zones.topKey.val} />
      </div>
    </div>
  );
};